...
class SearchedHiddenFilesDisplaySteps(
    ...
    fun searchResultsShown() {
        ...
        expectedFiles.forEachIndexed { index, file ->
            ...
            Shadows.shadowOf(Looper.getMainLooper()).idle()
            onView(
                withRecyclerView(com.alon.filesviewer.browser.ui.R.id.searchResults)
                    .atPosition(index)
            )
                .check(
                    matches(
                        hasDescendant(
                            allOf(
                                withId(com.alon.filesviewer.browser.ui.R.id.fileName),
                                withText(file.name)
                            )
                        )
                    )
    ...
    fun searchScreenShouldChangeDisplay() {
        Thread.sleep(4000)
        onView(withId(com.alon.filesviewer.browser.ui.R.id.searchResults))
            .check(matches(withRecyclerViewSize(expectedFiles.size)))
        expectedFiles.forEachIndexed { index, file ->
